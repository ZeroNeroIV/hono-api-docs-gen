...
function isExcluded(
  r: ParsedRoute,
  exclude: { method?: string; path?: string }[] | undefined,
): boolean {
  return (
    exclude?.some((e) => {
...
interface GroupedRoutes {
  [tag: string]: ParsedRoute[];
}

function groupByTag(routes: ParsedRoute[]): GroupedRoutes {
...
function renderSchemaTable(schema: SchemaDefinition, depth = 0): string {
...
function formatPropertyType(prop: SchemaProperty): string {
  let t: string = prop.type;
  if (prop.format) t += ` (${prop.format})`;
  if (prop.nullable) t += " | null";
  if (prop.type === "array" && prop.items) t = `${prop.items.type}[]`;
  if (prop.enum) t += ` [${prop.enum.join(", ")}]`;
  return t;
}

function renderSchemaExample(schema: SchemaDefinition): string {
...
function buildExampleFromSchema(schema: SchemaDefinition): unknown {
...
function buildPropertyExample(prop: SchemaProperty): unknown {
...
function renderParameters(params: ParameterDefinition[]): string {
...
function renderRequestBody(body: RequestBodyDefinition): string {
  let html = '<div class="request-body-section"><h4>Request Body';
  if (body.required) html += ' <span class="required-badge">required</span>';
  html += "</h4>";
  if (body.description) html += `<p class="section-desc">${esc(body.description)}</p>`;
  if (body.contentType)
...
function renderResponses(responses: Record<number, ResponseDefinition>): string {
...
function renderRouteCard(route: ParsedRoute, index: number): string {
  ...
  if (meta?.summary) html += `<span class="route-summary">${esc(meta.summary)}</span>`;
  if (meta?.deprecated)
...
// ─── Path param auto-extraction ─────────────────────────────────────────────

/** Extract parameter names from route paths like `/users/:id` or `/users/{id}` */
...
// ─── Body hint (field reference above textarea) ─────────────────────────────

function renderBodyHint(
  ...
  // For object schemas: render a compact field reference
  if (schema.type === "object" && schema.properties) {
    ...
    for (const [name, prop] of Object.entries(schema.properties)) {
      ...
      html += `<td>`;
      if (prop.description) html += `${esc(prop.description)}`;
      if (constraints) html += `${prop.description ? " — " : ""}${constraints}`;
      if (prop.default !== undefined) html += ` <span class="hint-default">Default: <code>${esc(String(prop.default))}</code></span>`;
      if (prop.example !== undefined) html += ` <span class="hint-example">e.g. <code>${esc(String(prop.example))}</code></span>`;
      html += `</td>`;
      html += `</tr>`;
    }
    ...
    // For array schemas: describe the expected item shape
    html += `<div class="body-hint-fields">`;
    ...
    if (schema.description) html += `<p class="body-hint-desc">${esc(schema.description)}</p>`;
    html += `</div>`;
  }
...
/** Render a hint table for an object schema's properties (reused for nested array items) */
function renderBodyHintTableForObject(schema: SchemaDefinition): string {
...
/** Build human-readable constraint text from schema property metadata */
function buildConstraintHints(prop: SchemaProperty): string {
  const parts: string[] = [];
...
function renderTryItOut(route: ParsedRoute, index: number): string {
  ...
  // ── Path parameters ───────────────────────────────────────────────────
  // Always auto-detect from the route path; merge with any declared metadata
  ...
  // Render declared params first (they have descriptions/types)
  ...
  // Then render auto-detected params not already declared
  for (const name of autoPathParams) {
    if (declaredPathNames.has(name)) continue;
  ...
  // ── Dynamic query parameters (add more) ───────────────────────────────
  ...
  html += "</div>"; // try-it-panel
  html += "</div>"; // try-it-out-section
  return html;
}

// ─── Main HTML generation ───────────────────────────────────────────────────

export function generateDocHtml(routes: ParsedRoute[], config: DocConfig): string {
  ...
  const tagOrder = [
    ...
    ...Object.keys(grouped).filter(
      (t) => !cfg.tags.some((ct) => ct.name === t),
    ),
  ];

  for (const tag of tagOrder) {
    const tagRoutes = grouped[tag];
    if (!tagRoutes?.length) continue;
    ...
    routeIndex -= tagRoutes.length; // reset for main content
  ...
  // Produce route data as JSON for the "try it out" JS
  const routeDataJson = JSON.stringify(
    filtered.map((r) => {
      // Merge declared path params with auto-detected ones
      ...
      return {
        ...
        bodyContentType:
          r.metadata?.requestBody?.contentType ?? "application/json",
      };
    }),
  );
...
function generateStyles(cfg: Required<DocConfig>): string {
  const t = cfg.theme;
  return `<style>
:root {
...
function generateScript(routeDataJson: string, serversJson: string): string {
  return `<script>
